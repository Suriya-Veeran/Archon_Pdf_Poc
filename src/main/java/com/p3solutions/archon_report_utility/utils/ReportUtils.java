...
public class ReportUtils implements ExecutableClass {
  ...
  public void createHalfDivider(DividerInputBean dividerInputBean) {
    ...
    canvas.lineTo(pdfPage.getPageSize().getWidth() - 35, dividerInputBean.getHeight());
    canvas.setLineWidth(dividerInputBean.getLineWidth());
    canvas.closePathStroke();
  }
  ...
  @Override
    ...
    LSBSteganography.encodeMessage(coverImage, "Nan dhan da leo... leo daaas");

    String modifiedImagePath = location + "/modified-cover-image.png"; // Adjust the path as necessary
    ...
    for (int i = 1; i <= document.getPdfDocument().getNumberOfPages(); i++) {
      ...
      pdfCanvas.addImage(imageData, x, y,  image.getImageHeight(), false,false);
