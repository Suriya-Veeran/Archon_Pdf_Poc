...
public class ReportUtils implements ExecutableClass {
  ...
  private void addFooterToPage(FooterInputBean footerInputBean, int pageIndex, int totalPages) {
  ...
  public void createDivider(DividerInputBean dividerInputBean) {
    ...
    canvas.setStrokeColor(hexaDecimalToRGB(dividerInputBean.getHexDecimal()));
  ...
  @Override
  public void createJobStatusFailureTable(String header, String value, Table table)
      throws IOException {
    ...
    if (!status) {
      ...
      Paragraph errorParagraph =
          new Paragraph(ERROR_MESSAGE_HEADER + "Schema ad_test_dbo2 is not found")
              .setFontSize(7)
  ...
  @Override
  public void createJobStatusTable(String header, String value, Table statusTable)
      throws IOException {
  ...
  private File getCroppedImage(File file, String name, int x, int y, int width, int height)
      throws IOException {
  ...
  @Override
  public void setHiddenWatermarkImage(String location, String imagePath, String message)
      throws IOException {
  ...
  public Table setOptimizationStatistics(
      OptimizationStatisticsBean optimizationStatisticsBean, Table optimizationSettingTable) {
  ...
  private Cell configureCell(ColumnInputBean columnInputBean, Cell cell, String content)
      throws IOException {
  ...
  private Cell headerColorDetermination(ColumnInputBean columnInputBean, Cell cell, String content)
      throws IOException {
    ...
    PdfFont font =
        columnInputBean.isJobSummaryHeader()
  ...
  @Override
  public void deleteTempFiles(File tempFilePath) {
    if (tempFilePath.isDirectory()) {
      for (File listFile : Objects.requireNonNull(tempFilePath.listFiles())) {
        deleteTempFiles(listFile);
      }
    }
    String message = tempFilePath.delete() ? "Deleted" : "Failed to delete";
    log.info(message);
  }
  ...
  public void deleteTempFilesUsingPath(Path tempFilePath) throws IOException {
    if (Files.isDirectory(tempFilePath)) {
      ...
      try {
        ...
        log.error("Failed to delete file: {}", tempFilePath, e);
      }
    }
  }
}
